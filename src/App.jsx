    ...
    <div className="app">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <h1>CM Autopilot</h1>
            <span className="sub">Rolling Stock · Corrective Maintenance</span>
          </div>
        </div>
      ...
      {tab === 'faults' && parsed && stats && (
        <main className="main">
          <section className="tiles">
            ...
            <Tile label="Noise suppressed" value={(stats.systemFiltered + stats.infoFiltered).toLocaleString()}
              note={`${stats.systemFiltered.toLocaleString()} TCMS · ${stats.infoFiltered.toLocaleString()} informational`} tone="muted" />
            <Tile label="Storms collapsed" value={stats.collapsed.toLocaleString()} note="repeat firings merged" tone="muted" />
            ...
            <Tile label="Repeat offenders" value={(stats.badActors || 0).toLocaleString()}
              note="same fault, same place, 3+ returns" tone="danger" />
          </section>
          ...
          <section className="reduction">
            ...
            <p><b>{stats.total.toLocaleString()}</b> log rows reduced to <b>{stats.incidents.toLocaleString()}</b> incidents
              — a <b>{stats.total ? (stats.total / Math.max(stats.incidents, 1)).toFixed(0) : 0}×</b> reduction in what a technician reads.</p>
          </section>

          <section className="controls">
            <div className="filters">
              <Select label="Severity" value={filterSev} onChange={setFilterSev} options={['All', ...SEV_ORDER.filter((s) => sums.bySeverity[s])]} />
              <Select label="Function" value={filterFn} onChange={setFilterFn} options={['All', ...Object.keys(sums.byFunction).sort()]} />
              <Select label="Trainset" value={filterTs} onChange={setFilterTs} options={['All', ...Object.keys(sums.byTrainset).sort()]} />
              <input className="search" placeholder="Search mnemonic, description, location…" value={query} onChange={(e) => setQuery(e.target.value)} />
            </div>
            <div className="toggles">
              ...
              <Toggle checked={opts.hideInformation} label="Hide informational" onChange={(v) => setOpts({ ...opts, hideInformation: v })} />
              <label className="gap">Storm gap
          ...
          <section className="split">
            <div className="tablewrap">
              <table>
                <thead>
                  <tr>
                    <th style={{ width: 44 }}>#</th><th style={{ width: 74 }}>Sev</th><th>Fault</th>
                    <th style={{ width: 84 }}>Train</th><th style={{ width: 76 }}>Loc</th>
                    <th style={{ width: 66 }}>Firings</th><th style={{ width: 66 }}>Returns</th>
                    <th style={{ width: 84 }}>Duration</th><th style={{ width: 128 }}>Last seen</th><th style={{ width: 60 }}>Score</th>
                  </tr>
                </thead>
                <tbody>
                  {visible.slice(0, 400).map((inc, n) => (
                    <tr key={inc.id} className={selected?.id === inc.id ? 'sel' : ''} onClick={() => setSelected(inc)}>
                      ...
                      <td>{inc.repeats >= 3
                        ? <span className="badactor" title="Returned on separate occasions — never actually fixed">×{inc.repeats}</span>
            ...
            <aside className="panel">
              {selected ? <RepairCard incident={selected} kbResult={lookup(selected.mnemonic)} />
                : <div className="empty"><h3>Select an incident</h3><p>The repair card is generated from the Level 3 troubleshooting manual — probable cause, remedial action, circuit breaker, and which manual to open next.</p></div>}
            </aside>
          </section>
        </main>
      )}
      ...
      <div className={`drop ${over ? 'over' : ''}`}
        ...
        {busy ? (
          ...
          <><div className="dropicon">↓</div>
            ...
            <p>All files for a trainset at once — <code>T5139_1.csv … T5139_8.csv</code>. They are stitched, de-duplicated and triaged.</p>
            <button className="cta">Choose files</button></>
        )}
      </div>
      <div className="kbnote">
        {kbReady ? <>✓ Knowledge base loaded — <b>152 IOS procedures</b> indexed from <i>Volume 3, Train Level Troubleshooting, Rev H</i></>
          : <>Loading troubleshooting knowledge base…</>}
      </div>
      <ul className="how">
        <li><b>1 · Triage</b> Fault storms collapsed, TCMS chatter suppressed, incidents ranked by severity × recurrence × recency × repeat.</li>
        <li><b>2 · Diagnose</b> Each incident matched to its manual procedure — cause, remedy, breaker, schematic.</li>
...
function Select({ label, value, onChange, options }) {
  return <label className="sel"><span>{label}</span><select value={value} onChange={(e) => onChange(e.target.value)}>{options.map((o) => <option key={o} value={o}>{o}</option>)}</select></label>;
}
